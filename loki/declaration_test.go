...
func TestTheDeclaredFieldSetIsTheOneTheResolverReads(t *testing.T) {
	...
	build := func(decl framework.ForeignContextDeclaration) logpipe.CloudContext {
		...
		return logpipe.NewCloudContext(graphs)
	}
	...
	if _, resolved := own.ResolveService(resourceName); !resolved {
	...
	if _, resolved := borrowed.ResolveService(resourceName); resolved {
