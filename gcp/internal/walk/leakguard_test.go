...
func TestMain(m *testing.M) {
	...
	// collector serves the protocol on its stdout and exits; running the suite
	// or the leak gate in it would write test output onto the protocol stream,
	// which is the exact corruption the transport rule exists to prevent.
