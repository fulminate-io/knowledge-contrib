...
//
// THIS COLLECTOR'S ENTIRE ENVIRONMENT BLOCK IS A CREDENTIAL REFERENCE, and that
// is the one way it differs from every other collector's. Its whole environment
// closure is two names and both are CREDENTIALS, so the block carries no literal
// at all: the entry names the variable as `"GITHUB_TOKEN": "${GITHUB_TOKEN}"` and
// the process serving the collect reads the value out of the operator's own
// environment at spawn. The value is in no file, and a serving process that does
// not hold the name is refused BY NAME rather than handed an empty credential.
...
const FamilyName = "github-actions"
...
const commandPlaceholder = "/usr/local/bin/knowledge-collector-github-actions"
...
func Worked() File {
	...
	return File{Collectors: map[string]Entry{
		FamilyName: {
			...
			Tool:    ToolName,
			// THE TOKEN BY REFERENCE, AND ONLY THE PRIMARY NAME. The two names in
			// this collector's closure are ALTERNATIVES — the second is consulted
			// when the first is unset — so an entry naming both would demand that
			// an operator hold two variables to authenticate once. The installer
			// writes whichever of them the installing shell holds; the documented
			// entry shows the one consulted first.
			...
			Behavior: &Behavior{
				...
				// content is a JSON document, and embedding one spends the vector
				// on its punctuation.
				...
				// BM25: all three. A keyword search for a workflow path, a
				// secret name or a runner label finds it in the content and
				// nowhere else, which is what an operator searches a CI/CD graph
				// for.
