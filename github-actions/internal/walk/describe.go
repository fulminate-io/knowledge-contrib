...
// ghgraph's declared resource and edge types, the environment from enventry's
...
func (c Collector) Describe() framework.Declaration {
	return framework.Declaration{
		...
		//
		// IT IS NOT ghgraph.ResourceTypes(). That is the resource-KIND floor —
		...
		NodeTypes:   []string{ghgraph.NodeType},
		EdgeTypes:   ghgraph.EdgeTypes(),
