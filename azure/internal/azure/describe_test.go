...
func TestDescribe_EveryEnumeratedEnvNameHasADisposition(t *testing.T) {
	enumerated := envNamesForTest(t, "linux")
	if len(enumerated) == 0 {
...
// envNamesForTest is the module's own environment enumeration, flattened to
// names. It is the SECOND symbol the disposition cross-check reads: the
// dispositions are a map beside it, and a name in one and not the other is the
// failure that row exists for.
func envNamesForTest(t *testing.T, goos string) []string {
	t.Helper()
	vars, err := EnvironmentNames(goos)
	if err != nil {
		t.Fatalf("enumerating the declared environment for %s: %v", goos, err)
	}
