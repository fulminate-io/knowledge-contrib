...
// context on every collect: runPipeline computes stream resolutions from it
// unconditionally, finds correlations with it when correlation is enabled, and
// assembleGraph turns both into the proxy nodes, EMITTED_BY edges and
// CORRELATES_WITH edges the result carries. It declared nothing, and a collector
// receives only what its entry declared, so the read was of an empty block on
// every collect an operator has ever run. The cross-graph half of this collector
// was dead by construction rather than by a wrong string — a different cause
// from the sibling collectors whose declarations named node types nothing emits,
// and the same consequence.
//
// THE RESOLUTIONS ARE COMPUTED WHATEVER THE CORRELATION FLAG SAYS. Only the
// correlation half is gated on cfg.CorrelationEnabled; the resolutions reach
// assembleGraph either way, so this declaration is load-bearing for the proxy
// half even with correlation turned off.
//
// THE FIELD SET IS THIS MODULE'S OWN AND IS NOT INTERCHANGEABLE. matchableNames
// indexes each declared node under three values: its `symbol_name`, its `name`
// metadata key and its `service` metadata key. A
...
// inventory family whose resources carry a name a Cloud Logging label can equal is
// worth declaring. Three do. `gcp` is the absence with a reason: the gcp collector
...
// `context` key is two levels inside `collectors` → `stackdriver`, and this
// module's documented entry is indented two spaces per level.
...
func RenderedContextBlock() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("stackdriver: rendering the declared context block: %w", err)
	}
