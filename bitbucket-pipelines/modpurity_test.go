...
// NOT enforce itself is a `require` — a module can depend on the client module
// and pull its whole surface in without importing anything private. That is the
// half this asserts.
//
...
// keeps the upstream spelling deliberately: the client and the server are not
// published there and must stay refused by their real names.
...
// the cache-key script names by explicit path and hard-errors on. Both halves are
...
// suite rather than only at the census, and which travels with the module into
// the published repository where the census does not run.
