...
//
// THIS MODULE'S FAN-OUT HAS A CANCELLATION ARM, and that arm is what this gate
// exists for: a walk cancelled mid-flight closes its work channel and waits, on
// every path, so no worker is left blocked on a channel nobody will write to.
// The walk suite cancels a walk mid-flight in the same run, so the gate has
// something to catch.
//
