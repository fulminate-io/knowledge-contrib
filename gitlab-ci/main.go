...
// Command gitlab-ci is a knowledge collector for GitLab CI/CD. It serves one
// MCP tool over stdio; a daemon spawns it, calls that tool with a collect id
// naming a GitLab group, and writes what it returns into a graph of its own.
//
// IT READS AND NEVER WRITES. Every call it makes is a list or a get, and it
// holds no operation that could change anything in a group.
//
// IT AUTHENTICATES ONLY FROM ITS ENVIRONMENT and takes no credential from its
// caller: there is no flag for one and no argument it reads. The names it
...
func main() {
	...
	collector := walk.Collector{API: glclients.API}
	if err := framework.ServeStdio(ctx, collector); err != nil {
		...
		fmt.Fprintf(os.Stderr, "gitlab-ci collector: %v\n", err)
		os.Exit(1)
	}
}
