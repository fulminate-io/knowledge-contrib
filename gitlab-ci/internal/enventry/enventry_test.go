...
// a consequence of how this module reads the environment: both resolvers take the
// lookup function as a PARAMETER and ask it for the declared names in turn, so
// there is no direct call carrying a literal name anywhere to find. What the
// census can establish — and what makes the claim true — is that the whole module
// contains exactly ONE FILE that touches the process environment at all.
//
...
// injected values — the provider client, the enumerations, the walk — so a second
// file reading the environment would be a second, undeclared input to a collector
// whose whole environment is what its config entry names.
const theOneFileThatReadsTheEnvironment = "internal/glclients/glclients.go"

// TestTheClosureIsExactlyThreeNames is the declaration itself.
func TestTheClosureIsExactlyThreeNames(t *testing.T) {
	names := enventry.Names()
	want := []string{"GITLAB_TOKEN", "GITLAB_PRIVATE_TOKEN", "GITLAB_URL"}
	if !slices.Equal(names, want) {
		t.Errorf("the declared closure is %v, want %v with the two credentials in consultation "+
			"order", names, want)
	}
	...
	for name, want := range map[string]enventry.Class{
		enventry.PrimaryTokenVariable:  enventry.ClassSecret,
		enventry.FallbackTokenVariable: enventry.ClassSecret,
		enventry.BaseURLVariable:       enventry.ClassSelector,
	} {
		if classes[name] != want {
			t.Errorf("%q is classed %q, want %q", name, classes[name], want)
		}
	}
	// THE SELECTOR IS WHY THIS COLLECTOR'S INSTALLED ENTRY HAS AN ENVIRONMENT
	// BLOCK AT ALL. A module whose whole closure was secret-class would carry
	// none, and the install suite has a separate case for that shape.
	if classes[enventry.BaseURLVariable] == enventry.ClassSecret {
		t.Error("the instance selector is classed as a credential; it names a host, and an " +
			"installer writes it as a literal")
	}
}

// TestTheTokenNamesAreTheCredentialsInConsultationOrder. The resolver walks this
// list, so its order is the answer to which variable wins.
func TestTheTokenNamesAreTheCredentialsInConsultationOrder(t *testing.T) {
	want := []string{"GITLAB_TOKEN", "GITLAB_PRIVATE_TOKEN"}
	if got := enventry.TokenNames(); !slices.Equal(got, want) {
		t.Errorf("the credential names are %v, want %v", got, want)
	}
	for _, name := range enventry.TokenNames() {
		if !enventry.IsDeclared(name) {
			t.Errorf("%q is a credential name the whole closure does not declare", name)
		}
	}
	if enventry.IsDeclared("GITLAB_HOST") {
		t.Error("a name this collector does not read is declared")
	}
}
...
func f() (string, string) {
	direct := os.Getenv("PLANTED_FOURTH_NAME")
	indirect := os.LookupEnv
	value, _ := indirect("PLANTED_FIFTH_NAME")
	return direct, value
}
`
	dir := t.TempDir()
	...
	if !slices.ContainsFunc(sites, func(s readSite) bool { return s.name == "PLANTED_FOURTH_NAME" }) {
