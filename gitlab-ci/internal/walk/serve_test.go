...
const stubModeEnv = "GITLAB_CI_COLLECTOR_SERVE_MODE"
...
// could never fire.
const plantedStdoutEnv = "GITLAB_CI_COLLECTOR_PLANT_STDOUT"

// serveAsCollector is called from TestMain in the CHILD. It serves this module's
// real collector over the framework's stdio entry point with a recorded provider,
// and never returns.
...
func dial(t *testing.T) *mcp.ClientSession {
	...
	client := mcp.NewClient(&mcp.Implementation{Name: "gitlab-ci-suite", Version: "v1"}, nil)
...
func TestTheAdvertisedSchemasSatisfyTheContract(t *testing.T) {
	...
	for _, want := range []string{`"id"`, `"params"`, `"max_pipeline_runs"`, `"max_deployments"`} {
		if !strings.Contains(advertised, want) {
			t.Errorf("the advertised input schema does not carry %s: %s", want, advertised)
		}
	}
	// The control: a name this collector's params do NOT have must be absent, so a
	// schema that mentioned everything would not pass.
...
func TestACollectOverStdioReturnsTheEnvelope(t *testing.T) {
	...
	res, err := session.CallTool(t.Context(), &mcp.CallToolParams{
		Name:      "collect",
		Arguments: map[string]any{"id": "acme", "params": map[string]any{"max_pipeline_runs": 5}},
	})
	if err != nil {
...
// TestAMalformedGroupIsRefusedOverTheTransport closes the loop on the validator:
// the refusal reaches the caller as a tool error rather than as an empty
// successful collect.
func TestAMalformedGroupIsRefusedOverTheTransport(t *testing.T) {
	session := dial(t)

	res, err := session.CallTool(t.Context(), &mcp.CallToolParams{
		Name:      "collect",
		Arguments: map[string]any{"id": "acme/plat form"},
	})
	if err != nil {
		t.Fatalf("calling collect over stdio: %v", err)
	}
	if !res.IsError {
		t.Fatal("a malformed group path was collected rather than refused")
	}
	if !strings.Contains(resultText(res), "letters, digits, underscores") {
...
func TestACapOfTheWrongKindIsRefusedBeforeTheWalkRuns(t *testing.T) {
	...
	res, err := session.CallTool(t.Context(), &mcp.CallToolParams{
		...
		Arguments: map[string]any{
			"id": "acme", "params": map[string]any{"max_pipeline_runs": "twenty"},
		},
	})
	if err != nil {
		// The SDK may surface a schema violation as a protocol error rather than as
		// a tool result; either is a refusal, and this is the arm that says so.
		if !strings.Contains(err.Error(), "max_pipeline_runs") &&
	...
	if !res.IsError {
		t.Fatal("a non-integer max_pipeline_runs was accepted")
	}
	text := resultText(res)
	if !strings.Contains(text, "max_pipeline_runs") && !strings.Contains(text, "integer") {
...
// have to stand between the SDK and the file descriptor, which is a fixture on the
// side of the seam under test. What an operator actually experiences is the
// handshake failing with a message about nothing they can act on, and that is
// reproducible end to end: the SAME child, with one line printed before it serves,
// cannot be dialed.
//
