...
// glgraph's declared resource and edge types, the environment from enventry's
...
func (c Collector) Describe() framework.Declaration {
	return framework.Declaration{
		...
		//
		// IT IS NOT glgraph.ResourceTypes(). That is the resource-KIND floor —
		...
		NodeTypes:   []string{glgraph.NodeType},
		EdgeTypes:   glgraph.EdgeTypes(),
...
func describedEnvironment() []framework.EnvDeclaration {
	...
	for _, name := range names {
		out = append(out, framework.EnvDeclaration{Name: name, Class: string(classes[name])})
	}
	return out
}
