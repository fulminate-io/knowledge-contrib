...
func TestDescribe_IsServable(t *testing.T) {
	if _, err := framework.NewServer[Params](&Collector{}); err != nil {
...
// envNamesForTest is the module's own environment enumeration for the platform
// an installer targets, out of cluster — the shape whose names an installed
// entry carries.
func envNamesForTest(t *testing.T) []string {
	t.Helper()
	names, err := EnvNames("linux", false)
	if err != nil {
		t.Fatalf("enumerating the declared environment: %v", err)
	}
	return names
}
